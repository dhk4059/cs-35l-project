...
import WrittenDiningReviewCard from "./WrittenDiningReviewCard.js";
...
const WrittenDiningReviews = ({ diningKey, resetList }) => {
  ...
  useEffect(() => {
    const getUsers = async () => {
      const allReviews = await getDocs(
        query(collection(firestore, diningKey), orderBy("timestamp", "desc"))
      );
      setAllReviews(
...
export default WrittenDiningReviews;
